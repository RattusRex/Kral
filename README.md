...
- `DELETE /api/admin/users/{id}` lets only a global **owner** delete another account. Related characters, inventories, calendars, chat, recruitment, shop, transfer, and project-membership rows are removed atomically; accounts that still own projects must transfer those projects first.
...
- `GET /api/projects/current/about` returns the selected project's publication feed and independent creator-information sidebar. Head administrators, project owners, and the global owner may manage this content; choosing a project opens this page first.
...
- Every busy-day journal entry displays its inclusive start/end date range and total day count in character, personal-hireling, and simulacrum calendars, including calendars opened by administrators.
